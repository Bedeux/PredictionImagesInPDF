...
def get_colors(imagePath):
    ...
    MostFrequentColor = ""
    SecondMostFrequentColor = ""
    
    if all_pixels: # If there is pixels :
...
image_names = os.listdir(path_to_project+"/1. Data/Labelisation/Textes Pixelisées/")
...
for file_name in image_names:
    n+=1
    print(n)
    path = path_to_project+"/1. Data/Labelisation/Textes Pixelisées/"+file_name
...
def create_characters_values(files_of_image):
    ...
    for imagei in files_of_image:  
        n+=1
        print(n)
        path = path_to_project+"/1. Data/Labelisation/Textes/" + str(imagei)
...
all_image = os.listdir(path_to_project+'/1. Data/Labelisation/Textes')
dico = create_characters_values(all_image)

target_values = {}
for image in all_image:
    target_values[image] = [0] # TEXT

for image in datas:
    if(image in dico):
        datas[image].extend(dico[image])
    if(image in target_values):
        datas[image].extend(target_values[image])
